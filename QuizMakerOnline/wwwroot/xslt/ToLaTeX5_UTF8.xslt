...
\bigskip
{\bf <xsl:value-of select="position()"/>.} <xsl:value-of select="@question"/><xsl:if test="@question!='' and @id_question_type!=2">~<xsl:if test="@points!=0 and @id_question_type!=3 and /Tests/@showPoints='true'">\hfill(<xsl:value-of select="@points"/><xsl:text> </xsl:text> <xsl:choose>
...
</xsl:if><xsl:if test="position()!=last()">~\hfill
</xsl:if>
</xsl:template>
</xsl:stylesheet>